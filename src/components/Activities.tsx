...
const Activities: React.FC = () => {
  ...
  return (
    <section id="activities" ref={sectionRef} className="py-20 bg-gray-50 dark:bg-gray-800 transition-colors duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {activities.map((activity, index) => (
            <div key={index} className="activity-card group">
              <div className="bg-white dark:bg-gray-700 rounded-2xl p-8 shadow-lg hover:shadow-2xl dark:shadow-gray-900/50 transition-all duration-500 transform hover:-translate-y-3 h-full">
                {/* Icon */}
                ...
                <div className="space-y-2">
                  {activity.features.map((feature, idx) => (
                    <div key={idx} className="flex items-center text-sm text-gray-500 dark:text-gray-400 transition-colors duration-300">
                      <div className="w-2 h-2 bg-blue-400 dark:bg-blue-300 rounded-full mr-3"></div>
                      {feature}
                    </div>
                  ))}
                </div>
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-blue-600/10 dark:from-blue-400/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl"></div>
              </div>
            </div>
          ))}
        </div>
